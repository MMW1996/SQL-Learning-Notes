...
case 
when seq_in_index = (
     select max(st1.seq_in_index) 
     from information_schema.statistics st1 
		 where st1.table_name = st.table_name 
			 and st1.index_schema = st.index_schema
			 and st1.index_name = st.index_name)
then ');'
		 else ''
		 end
) create_index
